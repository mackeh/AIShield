...
fn parse_rule(content: &str, source: &Path) -> Result<Rule, String> {
        ...
        match section {
            Section::Pattern => {
                ...
                if let Some(value) = trimmed.strip_prefix("- ") {
                    contains_patterns.push(strip_quotes(value).to_ascii_lowercase());
                }
            }
            Section::Negative => {
                if let Some(value) = trimmed.strip_prefix("- ") {
                    negative_patterns.push(strip_quotes(value).to_ascii_lowercase());
                }
            }
            Section::Fix => {
    ...
    if contains_patterns.is_empty() {
        return Err(format!("{}: missing `pattern.contains`", source.display()));
    }
